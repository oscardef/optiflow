...
def bulk_insert_purchases(purchases: list[dict], db: Session = Depends(get_db)):
    ...
    Note: Creates dummy inventory_item records for historical data with unique RFID tags
    """
    ...
    try:
        ...
        for purchase_data in purchases:
            ...
            # Use UUID to ensure unique RFID tags across multiple backfill runs
            unique_id = uuid.uuid4().hex[:12]
            dummy_item = InventoryItem(
                product_id=purchase_data['product_id'],
                rfid_tag=f"HIST_{purchase_data['product_id']}_{unique_id}",
...
def clear_analytics_data(db: Session = Depends(get_db)):
    ...
    NOTE: This keeps Products and real InventoryItems intact, as they are needed
    for the simulation to function. Only historical analytics data and dummy
    inventory items (created for backfill) are removed.
    
    WARNING: This is a destructive operation and cannot be undone.
    """
    try:
        from ..models import PurchaseEvent, StockSnapshot, InventoryItem
        
        # Count records before deletion
        purchase_count = db.query(PurchaseEvent).count()
        snapshot_count = db.query(StockSnapshot).count()
        dummy_items_count = db.query(InventoryItem).filter(
            InventoryItem.rfid_tag.like('HIST_%')
        ).count()
        ...
        # Delete dummy inventory items created for historical backfill
        ...
        logger.info(f"Cleared {purchase_count} purchase events, {snapshot_count} stock snapshots, and {dummy_items_count} dummy items")
        
        # Signal the simulation to reset analytics tracker state via MQTT
        try:
            import paho.mqtt.publish as publish
            mqtt_broker = os.getenv("MQTT_BROKER", "localhost")
            mqtt_port = int(os.getenv("MQTT_PORT", "1883"))
            mqtt_topic = os.getenv("MQTT_TOPIC_CONTROL", "optiflow/control")
            
            publish.single(
                mqtt_topic,
                payload="RESET_ANALYTICS",
                hostname=mqtt_broker,
                port=mqtt_port,
                qos=1
            )
            logger.info(f"Sent RESET_ANALYTICS command to simulation via MQTT")
        except Exception as mqtt_error:
            logger.warning(f"Could not send MQTT reset command (simulation may need manual restart): {mqtt_error}")
        ...
        return {
            "status": "success",
            "message": f"Cleared {purchase_count} purchase events, {snapshot_count} stock snapshots, and {dummy_items_count} dummy items. {products_remaining} products and {items_remaining} real items remain.",
            "purchases_deleted": purchase_count,
            "snapshots_deleted": snapshot_count,
            "dummy_items_deleted": dummy_items_count,
